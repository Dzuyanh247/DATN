(() => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        async function openConversation(id) { if (!id) return resetEmptyState(); try { const data = await request(`/AdminChat/conversations/${id}/messages`); activeId = id; activeConversation = data.conversation || {}; placeholder.classList.add('d-none'); room.classList.remove('d-none'); root.classList.add('room-open'); setText('admin-chat-name', activeConversation.name || 'Khách hàng'); setText('admin-chat-avatar', (activeConversation.name || 'K').trim()[0]?.toUpperCase() || 'K'); setText('admin-chat-contact', [activeConversation.email, activeConversation.phone].filter(Boolean).join(' • ') || 'Chưa có thông tin liên hệ'); setStatus(activeConversation.status); setAssignment(activeConversation); showContext(activeConversation); messages.replaceChildren(); (data.messages || []).forEach(addMessage); appendAssignmentSystemMessage(activeConversation, 'current-assignment'); if (statusValue(activeConversation.status) === 'closed') appendSystemMessage('✓ Hội thoại đã được đóng', `closed-${activeId}`, activeConversation.closedAt || activeConversation.createdAt); scrollToBottom(true); const found = conversations.find(x => x.id === id); if (found) found.unreadCount = 0; renderList(); } catch (error) { showError(error.message); } }
        async function assign(staffId) { if (!activeId || !activeConversation) return; try { const previousStaffId = activeConversation.assignedStaffId; const data = await request(`/AdminChat/conversations/${activeId}/assign`, { method: 'POST', body: JSON.stringify({ staffId: staffId || null }) }); activeConversation.assignedStaffId = data.assignedStaffId; activeConversation.assignedStaffName = data.assignedStaffName; activeConversation.needsStaff = false; setAssignment(activeConversation); showContext(activeConversation); appendSystemMessage(`🎧 ${data.assignedStaffName} ${previousStaffId && previousStaffId !== data.assignedStaffId ? 'đã tiếp nhận cuộc hội thoại này' : 'sẽ hỗ trợ bạn từ bây giờ'}`, `assignment-live-${Date.now()}`); await loadConversations(); } catch (error) { showError(error.message); } }
        ...
        closeButton?.addEventListener('click', () => { if (!activeId || !activeConversation) return; window.kkConfirm({ title: 'Đóng hội thoại', message: 'Bạn có chắc muốn đóng hội thoại này? Tin nhắn vẫn được lưu lại.', confirmText: 'Đóng hội thoại', cancelText: 'Hủy', type: 'warning', onConfirm: async () => { try { await request(`/AdminChat/conversations/${activeId}/close`, { method: 'POST', body: '{}' }); setStatus('Closed'); activeConversation.status = 'Closed'; appendSystemMessage('✓ Hội thoại đã được đóng', `closed-live-${Date.now()}`); await loadConversations(); } catch (error) { showError(error.message); } } }); });
