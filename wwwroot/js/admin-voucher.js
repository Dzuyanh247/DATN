...
const code=document.getElementById('Code'),type=document.getElementById('DiscountType'),value=document.getElementById('DiscountValue'),max=document.getElementById('MaxDiscountAmount'),min=document.getElementById('MinimumOrderAmount'),orderMax=document.getElementById('MaxOrderAmount'),qty=document.getElementById('Quantity'),unlimited=document.getElementById('UnlimitedQuantity'),help=document.getElementById('discountValueHelp'),maxGroup=document.getElementById('maxDiscountGroup'),preview=document.getElementById('voucherPreview'),start=document.getElementById('StartDate'),end=document.getElementById('EndDate');
...
const c=code.value||'MÃVOUCHER';const discount=number(value);const minText=money(min.value||'0');const orderMaxText=number(orderMax?.value)>0?money(orderMax.value):'Không giới hạn';const maxText=number(max?.value)>0?`Tối đa ${money(max.value)}`:'Không giới hạn';const qtyText=unlimited?.checked?'Không giới hạn':(qty?.value?Number(qty.value).toLocaleString('vi-VN'):'Theo số lượng nhập');const startText=dateText(start?.value);const endText=dateText(end?.value);
setPreview('code',c);setPreview('type',isPercent?'Giảm theo phần trăm':'Giảm số tiền cố định');setPreview('value',isPercent?`${discount||0}% · ${maxText}`:money(value));setPreview('minimum',minText);setPreview('maximum',orderMaxText);setPreview('quantity',qtyText);setPreview('time',startText&&endText?`${startText} → ${endText}`:'Chọn ngày bắt đầu và kết thúc');}
[value,max,min,orderMax].forEach(el=>{el&&el.addEventListener('input',()=>formatMoney(el));});
root.addEventListener('input',sync);root.addEventListener('change',sync);root.closest('form')?.addEventListener('submit',()=>{[value,max,min,orderMax].forEach(el=>{if(el)el.value=digits(el.value);});if(qty&&unlimited?.checked){qty.disabled=false;qty.value='2147483647';}});
[value,max,min,orderMax].forEach(formatMoney);sync();
})();
