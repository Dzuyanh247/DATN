...
public class AdminComponentsController : Controller
{
    ...
    public async Task<IActionResult> Create(AdminComponentUpsertVm vm)
    {
        ...
        {
            Name = vm.Name,
            ProductCode = $"LK-{Guid.NewGuid():N}"[..16],
            ...
            StockQuantity = vm.StockQuantity!.Value,
            ...
            Specifications = vm.Specifications ?? string.Empty,
            IsActive = vm.IsActive,
            IsInStock = vm.StockQuantity.Value > 0,
    ...
    public async Task<IActionResult> Edit(AdminComponentUpsertVm vm)
    {
        ...
        product.Name = vm.Name; product.Brand = string.IsNullOrWhiteSpace(vm.Brand) ? null : vm.Brand.Trim(); product.ProductType = ProductKinds.Component; product.ComponentType = vm.ComponentType;
        product.Price = vm.Price!.Value; product.DiscountPrice = vm.DiscountPrice; product.SalePrice = vm.DiscountPrice; product.StockQuantity = vm.StockQuantity!.Value;
        ...
        product.Specifications = vm.Specifications ?? string.Empty; product.IsActive = vm.IsActive; product.IsInStock = vm.StockQuantity.Value > 0; product.Slug = BuildSlug(vm.Name);
    ...
    private async Task<AdminComponentUpsertVm?> BuildVmAsync(int? id = null)
    {
        ...
        vm.Id = p.Id; vm.Name = p.Name; vm.Brand = p.Brand; vm.ComponentType = string.IsNullOrWhiteSpace(p.ComponentType) ? ComponentTypes.Other : p.ComponentType; vm.Price = p.Price; vm.DiscountPrice = p.DiscountPrice ?? p.SalePrice; vm.StockQuantity = p.StockQuantity; vm.WarrantyMonths = p.WarrantyMonths; vm.CategoryId = p.CategoryId; vm.Description = p.Description; vm.Specifications = p.Specifications; vm.IsActive = p.IsActive; vm.ThumbnailImageUrl = p.ThumbnailImage;
